/*!
	...
	@note draw functions do not draw to the screen,  only draw to the frame buffer, 
		you must call updateScreen to draw the frame buffer to the screen

	TODO: cut down ST7735_TFT library to only include functions used in this wrapper
*/
...
/*!
	...
	@note This function does not draw to the screen, it only draws to the frame buffer, 
		you must call updateScreen to draw the frame buffer to the screen
*/
void frameBufferDrawBitmap(uint8_t *frameBuffer, uint8_t *bitmap, int x, int y, int w, int h) {
	// TODO: treat some color value as transparent so we can draw non-rectangular bitmaps
	...
	for(int i = 0; i < w; i++) { //loop through width
		for(int j = 0; j < h; j++) { //loop through height
			//only draw in bounds
			if(x + i < 0 || x + i >= 128 || y + j < 0 || y + j >= 160) {
				continue;
			}
...
/*!
	@brief Draw char to the frame buffer
	@param frameBuffer Pointer to the frame buffer, uint8_t array, 2 byte per pixel
	@param letter char to draw
	@param x X position of the letter
	@param y Y position of the letter
...
/*!
	@brief set pixel in the frame buffer
	@param frameBuffer Pointer to the frame buffer, uint8_t array, 2 byte per pixel
	@param x X position of the text
	@param y Y position of the text
...
//updates the screen with the frame buffer
